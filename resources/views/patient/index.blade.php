<x-app-layout title="Diet - Hasta Listesi">
            ...
            <div class="w-full overflow-x-auto">
                <table class="w-full whitespace-no-wrap">
                    ...
                    <tr class="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
                        ...
                        <th class="px-4 py-3">Diyetisyen</th>
                        <th class="px-4 py-3">E-posta</th>
                        <th class="px-4 py-3">Hastalık</th>
                        <th class="px-4 py-3">Diyet Metodu</th>
                    ...
                    @foreach($data as $patient)
                        ...
                        <td class="px-4 py-3 text-sm">
                            @if(isset($patient->dietitian->name))
                        ...
                        <td class="px-4 py-3 text-sm">
                            @if(isset($patient->disease->name))
                            {{$patient->disease->name}}
                            @endif
                        </td>
                        <td class="px-4 py-3 text-sm">
                            <select class="indexDietMethodSelect block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600
                ...
                focus:shadow-outline-purple dark:focus:shadow-outline-gray"
                                    name="diet_method_id" data-id="{{$patient->id}}">
                                <option value="">Seçiniz</option>
                                @foreach(\App\Models\DietMethod::all() as $method)
                                    <option {!! $patient->diet_method_id == $method->id ? 'selected' : ''!!} value="{{$method->id}}" >{{$method->name}}</option>
                                @endforeach
                            </select>
                        </td>
                        <td class="px-4 py-4 text-xs text-center text-purple-700 flex">
                            <div class="mr-6">
                                <a href="/patient/{{$patient->id}}/edit" class="inline-block">
                            ...
                            <div>
                                <form action="/patient/{{$patient->id}}" method="POST" class="inline-block">
                                    @csrf @method('DELETE')
                                    <button type="submit">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
