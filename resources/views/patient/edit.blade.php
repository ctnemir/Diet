<x-app-layout title="Diet - Hasta Güncelleme">
    <div class="container grid px-6 mx-auto">
        ...
        <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
            <label class="block text-sm">
                <span class="text-gray-700 dark:text-gray-400"><span class="text-red-700">*</span>Ad Soyad</span>
                ...
                form-input" name="name" value="{{$data->name}}"/>
            </label>
            <label class="block text-sm mt-4">
                <span class="text-gray-700 dark:text-gray-400">GSM</span>
                <input class="block w-full mt-1 text-sm dark:border-gray-600
                dark:bg-gray-700 focus:border-purple-400 focus:outline-none
                focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray
                form-input" name="gsm" type="tel" value="{{$data->gsm}}"/>
            </label>
            <label class="block text-sm mt-4">
                <span class="text-gray-700 dark:text-gray-400">E-posta Adresi</span>
                <input class="block w-full mt-1 text-sm dark:border-gray-600
                dark:bg-gray-700 focus:border-purple-400 focus:outline-none
                focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray
                form-input" type="email" name="email" value="{{$data->email}}"/>
            </label>
            <label class="block mt-4 text-sm">
                ...
                name="disease_id">
                    ...
                    @foreach(\App\Models\Disease::all() as $disease)
                        <option {!!$data->disease_id == $disease->id ? 'selected' : "" !!} value="{{$disease->id}}">{{$disease->name}}</option>
                    @endforeach
                </select>
            </label>
            <label class="block mt-4 text-sm">
                ...
                focus:shadow-outline-purple dark:focus:shadow-outline-gray"
                        name="diet_method_id">
                    <option value="">Seçiniz</option>
                    @foreach(\App\Models\DietMethod::all() as $method)
                        <option {!! $data->diet_method_id == $method->id ? 'selected' : ''!!} value="{{$method->id}}" >{{$method->name}}</option>
                    @endforeach
                </select>
            </label>
            <label class="block mt-4 text-sm">
